...
- `img:<mediaId>` → the original `Blob` for each media item (images/video/audio).
- `thumb:<mediaId>` → for **images only**, a small JPEG thumbnail (≤800px long edge,
  q0.8). Generated by `makeThumbnail` and written alongside the original by
  `saveImageMedia(id, blob, kind)` (used by both add paths: `App.attachFiles` and the
  composer). It is a parallel storage key, NOT a field on the memory object.
- **Thumbnail vs original — the memory-cost rule:** cards (`useThumb` in `MemoryCard`)
  and the orbit (`photoItem` → `thumbURL`) load the THUMBNAIL; **only the Lightbox**
  (`useImage`) loads the full-res original. So the timeline/orbit never decode or hold
  full-resolution bitmaps.
- `thumbURL(id)` → cached object URL for the thumbnail; **lazily generates + persists**
  one from the original for memories saved before thumbnails existed (falls back to the
  original if a blob can't be decoded, e.g. HEIC). `imageURL(id)` → the original (lightbox).
- **Releasing memory.** Object URLs are cached in `urlCache` (keyed by storage key) and
  reused across mounts — revoked only when the data is truly gone: `deleteImage` revokes +
  deletes both `img:` and `thumb:`; the Lightbox revokes the heavy original
  (`revokeOriginal`) on close (the card thumbnail stays). `InfiniteMemoryCanvas` disposes
  THREE textures + evicts `textureCache` for URLs no longer in the live media set (deferred
  ~1.5s so planes have swapped first), so VRAM doesn't grow unbounded.
